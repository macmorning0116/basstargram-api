...
class GptClientTest {
  ...
  void analyze_whenMultipartContentTypeIsNull_shouldFallbackToImageJpeg() throws Exception {
    ...
    MockMultipartFile image =
        new MockMultipartFile(
            "image", "test.bin", null, "fake-image-bytes".getBytes(StandardCharsets.UTF_8));
  ...
  void analyze_whenImageGetBytesThrows_shouldThrowGptApiError() throws Exception {
    ...
    FishingException ex =
        assertThrows(FishingException.class, () -> gptClient.analyze(badFile, dayWeather()));
  ...
  private MockMultipartFile jpegImage(String bytes) {
    return new MockMultipartFile(
        "image", "test.jpg", "image/jpeg", bytes.getBytes(StandardCharsets.UTF_8));
  }
